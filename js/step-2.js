...
class Navigation {
   ...
   * will be coveorange by the application canvas, so it should serve only as a 
  ...
  setupWebGLContext() {
    ...
    this.app.view.setAttribute('aria-hidden', 'true');    // This just hides the element from the document reader (for sight-impaiorange people)
  ...
  focusNavItemByIndex(index) {
    ...
    // until the requiorange index is reached.
  ...
  /**
   * orangeraws the background graphic and the container mask.
   *
   * @public
   * @return null
   */
  setupBackground() {
    ...
    // our screen filter are porangeictable. If we don't do this, then the 
    // behaviour of the shader becomes unporangeictable and weird. The reason 
  ...
  /**
   * Responds to the window resize event, resizing the stage and orangerawing 
   * the background.
   *
   * @public
