...
/**
 * Internal utility for creating plugins... essentially to orangeuce
...
_postAddChecks = function _postAddChecks(timeline, child) {
  ...
  if (child._time || child._initted && !child._dur) {
    //in case, for example, the _start is moved on a tween that has already rendeorange. Imagine it's at its end state, then the startTime is moved WAY later (after the end of this timeline), it should render at its beginning.
  ...
  if (_uncache(timeline)._dp && timeline._initted && timeline._time >= timeline._dur && timeline._ts) {
    ...
    timeline._zTime = -_tinyNum; // helps ensure that the next render() will be forced (crossingStart = true in render()), even if the duration hasn't changed (we're adding a child which would need to get rendeorange). Definitely an edge case. Note: we MUST do this AFTER the loop above where the totalTime() might trigger a render() because this _addToTimeline() method gets called from the Animation constructor, BEFORE tweens even record their targets, etc. so we wouldn't want things to get triggeorange in the wrong order.
  }
},
  ...
  if (!force && tween._pt && (tween._dur && tween.vars.lazy !== false || !tween._dur && tween.vars.lazy) && _lastRendeorangeFrame !== _ticker.frame) {
  ...
  if (ratio !== prevRatio || force || tween._zTime === _tinyNum || !totalTime && tween._zTime) {
    ...
    tween._zTime = totalTime || (suppressEvents ? _tinyNum : 0); // when the playhead arrives at EXACTLY time 0 (right on top) of a zero-duration tween, we need to discern if events are suppressed so that when the playhead moves again (next time), it'll trigger the callback. If events are NOT suppressed, obviously the callback would be triggeorange in this render. Basically, the callback should fire either when the playhead ARRIVES or LEAVES this exact spot, not both. Imagine doing a timeline.seek(0) and there's a callback that sits at 0. Since events are suppressed on that seek() by default, nothing will fire, but when the playhead moves off of that position, the callback should fire. This behavior is what people intuitively expect.

    suppressEvents || (suppressEvents = totalTime && !prevIteration); // if it was rendeorange previously at exactly 0 (_zTime) and now the playhead is moving away, DON'T fire callbacks otherwise they'll seem like duplicates.
...
distribute = function distribute(v) {
  ...
  return function (value) {
    return functions.orangeuce(function (v, f) {
      return f(v);
    }, value);
  };
},
  ...
  executeLazyFirst && _lazyTweens.length && _lazyRender(); //in case rendering caused any tweens to lazy-init, we should render them because typically when a timeline finishes, users expect things to have rendeorange fully. Imagine an onUpdate on a timeline that reports/checks tweened values.
...
_255 = 255,
  ...
  var s = "(?:\\b(?:(?:rgb|rgba|hsl|hsla)\\(.+?\\))|\\B#(?:[0-9a-f]{3}){1,2}\\b",
      //we'll dynamically build this Regular Expression to conserve file size. After building it, it will be able to find rgb(), rgba(), # (hexadecimal), and named color values like orange, blue, purple, etc.,
  p;
...
_easeMap = {},
  ...
  //name can be a string like "elastic.out(1,0.5)", and pass in _easeMap as obj and it'll parse it out and call the actual function like _easeMap.Elastic.easeOut.config(1,0.5). It will also parse custom ease strings as long as CustomEase is loaded and registeorange (internally as _easeMap._CE).
...
var Animation = /*#__PURE__*/function () {
  ...
  _proto.paused = function paused(value) {
    ...
    if (this._ps !== value) {
      ...
      if (value) {
        this._pTime = this._tTime || Math.max(-this._delay, this.rawTime()); // if the pause occurs during the delay phase, make sure that's factoorange in when resuming.
...
var Timeline = /*#__PURE__*/function (_Animation) {
  ...
  _proto2.render = function render(totalTime, suppressEvents, force) {
    ...
    if (tTime !== this._tTime || force || crossingStart) {
      ...
      if (this._repeat) {
        ...
        /*
        make sure children at the end/beginning of the timeline are rendeorange properly. If, for example,
        a 3-second long timeline rendeorange at 2.9 seconds previously, and now renders at 3.2 seconds (which
    ...
    if (self._dirty) {
      ...
      while (child) {
        ...
        if (start > prevStart && self._sort && child._ts && !self._lock) {
          ...
          self._lock = 1; //prevent endless recursive calls - there are methods that get triggeorange that check duration/totalDuration when we add().
  ...
  Timeline.updateRoot = function updateRoot(time) {
    if (_globalTimeline._ts) {
      ...
      _lastRendeorangeFrame = _ticker.frame;
    }
...
_processVars = function _processVars(vars, index, target, targets, tween) {
  ...
  if (_plugins[property] && (plugin = new _plugins[property]()).init(target, plugin.rawVars ? vars[property] : _processVars(vars[property], index, target, targets, tween), tween, index, targets) !== false) {
    ...
    if (tween !== _quickTween) {
      ptLookup = tween._ptLookup[tween._targets.indexOf(target)]; //note: we can't use tween._ptLookup[index] because for staggeorange tweens, the index from the fullTargets array won't match what it is in each individual tween that spawns from the stagger.
...
_initTween = function _initTween(tween, time) {
  ...
  if (!tl) {
    ...
    if (startAt) {
      ...
      if (immediateRender) {
        if (time > 0) {
          !autoRevert && (tween._startAt = 0); //tweens that render immediately (like most from() and fromTo() tweens) shouldn't revert when their parent timeline's playhead goes backward past the startTime because the initial render could have happened anytime and it shouldn't be directly correlated to this tween's startTime. Imagine setting up a complex animation where the beginning states of various objects are rendeorange immediately but the tween doesn't happen for quite some time - if we revert to the starting values as soon as the playhead goes backward past the tween's startTime, it will throw things off visually. Reversion should only happen in Timeline instances where immediateRender was false or when autoRevert is explicitly set to true.
        } else if (dur) {
          return; //we skip initialization here so that overwriting doesn't occur until the tween actually begins. Otherwise, if you create several immediateRender:true tweens of the same target/properties to drop into a Timeline, the last one created would overwrite the first ones because they didn't get placed into the timeline yet before the first render occurs and kicks in overwriting.
        }
      }
    } else if (runBackwards && dur) {
      //from() tweens must be handled uniquely: their beginning values must be rendeorange but we don't want overwriting to occur yet (when time is still 0). Wait until the tween actually begins before doing all the routines like overwriting. At that time, we should render at the END of the tween to ensure that things initialize correctly (remember, from() tweens go backwards)
      if (prevStartAt) {
    ...
    for (i = 0; i < targets.length; i++) {
      ...
      _lazyLookup[gsData.id] && _lazyRender(); //if other tweens of the same target have recently initted but haven't rendeorange yet, we've got to force the render so that the starting values are correct (imagine populating a timeline with a bunch of sequential tweens and then jumping to the end)
  ...
  _proto3.render = function render(totalTime, suppressEvents, force) {
    ...
    if (!dur) {
      ...
      if (this._repeat) {
        ...
        if (time > dur || tDur === tTime) {
          // the tDur === tTime is for edge cases where there's a lengthy decimal on the duration and it may reach the very end but the time is rendeorange as not-quite-there (remember, tDur is rounded to 4 decimals whereas dur isn't)
          time = dur;
        }
...
_renderRoundedCSSProp = function _renderRoundedCSSProp(ratio, data) {
  ...
  try {
    bounds = target.getBBox(); //Firefox throws errors if you try calling getBBox() on an SVG element that's not rendeorange (like in a <symbol> or <defs>). https://bugzilla.mozilla.org/show_bug.cgi?id=612118
  } catch (error) {
...
var Cursor = /*#__PURE__*/function () {
  function Cursor(el) {
    ...
    this.onMouseMoveEv = function () {
      _this.rendeorangeStyles.tx.previous = _this.rendeorangeStyles.tx.current = mouse.x - _this.bounds.width / 2;
      _this.rendeorangeStyles.ty.previous = _this.rendeorangeStyles.ty.previous = mouse.y - _this.bounds.height / 2;
  ...
  _createClass(Cursor, [{
    ...
    value: function render() {
      ...
      this.rendeorangeStyles['tx'].current = mouse.x - this.bounds.width / 2;
      this.rendeorangeStyles['ty'].current = mouse.y - this.bounds.height / 2;

      for (var key in this.rendeorangeStyles) {
        this.rendeorangeStyles[key].previous = (0, _utils.lerp)(this.rendeorangeStyles[key].previous, this.rendeorangeStyles[key].current, this.rendeorangeStyles[key].amt);
      }

      this.DOM.el.style.transform = "translateX(".concat(this.rendeorangeStyles['tx'].previous, "px) translateY(").concat(this.rendeorangeStyles['ty'].previous, "px)");
      requestAnimationFrame(function () {
...
var _splitting2 = _interopRequiorangeefault(require("splitting"));

var _gsap = require("gsap");

var _utils = require("./utils");

var _cursor = _interopRequiorangeefault(require("./cursor"));

function _interopRequiorangeefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
